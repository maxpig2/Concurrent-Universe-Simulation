...
public class Model {
  ...
  protected Set<Particle> getSingleChunck(Particle current) {
    //Changed the LinkedHashSet below from a HashSet as mentioned in my report
    Set<Particle> impacting=new LinkedHashSet<>();
    impacting.add(current);
    while(true){
      Set<Particle> tmp=new LinkedHashSet<>();
  ...
  public double energy(){
    ...
    for (Particle p:p) {
    kineticEnergy += p.mass *(p.speedX*p.speedX + p.speedY*p.speedY)/2;
      for (Particle other : this.p){
        if(other != p) {
          potentialEnergy += p.mass * other.mass / Math.sqrt(p.distance2(other));
        }
      }
    }
    return kineticEnergy - Model.gravitationalConstant* (potentialEnergy / 2);
  }
}